...
from image_proc.image_proc import StereoCamera, StereoCameraCalibrationData
...
def main():
    ...
    target_frames = 250
    combined_size = (1280, 480)  # Combined size for side-by-side stereo feed
    baseline = 0.06  # 6mm baseline - adjust this to match your camera setup
    
    # Initialize calibrator and camera
    calibrator = StereoCameraCalibrator(
        checkerboard_size=(8, 6),    # 9x7 checkerboard
        square_size=0.019,           # 19mm squares
        combined_size=combined_size,
        target_frames=target_frames,
        baseline=baseline            # Distance between camera centers
    )
    
    print("Opening stereo camera...")
    try:
        camera = StereoCamera(combined_size=combined_size)
    ...
    try:
        while frame_count < target_frames:
            ...
            if success:
                frame_count += 1
                print(f"\rSuccess! Captured frame {frame_count}/{target_frames}", end='')
    ...
    if frame_count > 0:
        ...
        try:
            ...
            output_file = 'calibration_results/stereo_calibration.json'
            
            with open(output_file, 'w') as f:
                json.dump(params, f, indent=4)
                
            print(f"\nCalibration complete! Parameters saved to '{output_file}'")
